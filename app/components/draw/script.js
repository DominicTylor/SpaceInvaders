...
export default class Draw {
    ...
    initGameObject () {
        this.tank = {
            ...
            x: (this.screen.width - this.taSprite.w) / 2,
            y: this.screen.height - (30 + this.taSprite.h),
        }
        ...
        [1, 0, 0, 2, 2].forEach((item, i) => {
                for (let j = 0; j < 10; j++) {
                    this.aliens.push({
                        sprite: this.alSprite[item],
                        x: 30 + j*30 + [0, 4, 0][item],
                        y: 30 + i*30,
                        w: this.alSprite[item][0].w,
                        h: this.alSprite[item][0].h
                    });
                }
            });
    }
    ...
    drawSprite (sp, x, y) {
        this.context.drawImage(this.spriteImg, sp.x, sp.y, sp.w, sp.h, x, y, sp.w, sp.h);
    };
